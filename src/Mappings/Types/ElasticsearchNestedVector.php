...
class ElasticsearchNestedVector extends TypesNested implements Type
{
    ...
    protected function mapSimilarityToScript(VectorSimilarity $similarity): string
    {
        ...
        return match ($similarity) {
            VectorSimilarity::Cosine => sprintf("cosineSimilarity(params.query_vector, '_embeddings.%s.vector') + 1.0", $fullPath),
            VectorSimilarity::DotProduct => sprintf("dotProduct(params.query_vector, '_embeddings.%s.vector')", $fullPath),
            VectorSimilarity::Euclidean => sprintf("1 / (1 + l2norm(params.query_vector, '_embeddings.%s.vector'))", $fullPath),
            VectorSimilarity::MaxInnerProduct => sprintf("dotProduct(params.query_vector, '_embeddings.%s.vector')", $fullPath),
        };
    }
